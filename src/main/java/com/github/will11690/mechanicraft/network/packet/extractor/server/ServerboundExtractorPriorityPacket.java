...
public class ServerboundExtractorPriorityPacket {
    ...
    public ServerboundExtractorPriorityPacket(int sideSelectID, int setPriority,  BlockPos blockPos, UUID playerUUID) {
    	
    	validMessage = true;
        ServerboundExtractorPriorityPacket.sideSelectID = sideSelectID;
        ServerboundExtractorPriorityPacket.setPriority = setPriority;
        ServerboundExtractorPriorityPacket.blockPos = blockPos;
        ServerboundExtractorPriorityPacket.playerUUID = playerUUID;
    }
    
    public static ServerboundExtractorPriorityPacket decode(PacketBuffer buffer) {
    	
        return new ServerboundExtractorPriorityPacket(buffer.readInt(), buffer.readInt(), buffer.readBlockPos(), buffer.readUUID());
    }
    ...
    public static void onMessageReceived(final ServerboundExtractorPriorityPacket message, Supplier<NetworkEvent.Context> ctxSupplier) {
    ...
    private static void processMessage(ServerboundExtractorPriorityPacket message, Supplier<NetworkEvent.Context> ctxSupplier) {
    	...
    	ctxSupplier.get().enqueueWork(() -> {
    		...
    		if(tile instanceof TileEntityT1EnergyExtractor && playerUUID == playerID) {
    			...
    			tileT1Extractor.setSidePriority(sideSelectID, setPriority);
    		}

	...
	public void oneUpgradeModifier(int burnTime, int energyGen, ItemStack upgrade) {
		...
		if(totalSpeed > 0 || totalEfficiency > 0) {
			...
			if(totalSpeed > 0 && totalEfficiency == 0) {
	    		...
	    		this.totalEnergyGen = onlySpeedUpgradeEfficiency(energyGen, totalSpeed);
	    		this.totalBurnTime = onlySpeedUpgradeSpeed(burnTime, totalSpeed);
	    		this.onUpgradeChanged();
					
			} else if(totalEfficiency > 0 && totalSpeed == 0) {
        			
        		this.totalEnergyGen = onlyEfficiencyUpgradeEfficiency(energyGen, totalEfficiency);
        		this.totalBurnTime = onlyEfficiencyUpgradeSpeed(burnTime, totalEfficiency);
        		this.onUpgradeChanged();
	...
	public void twoUpgradeModifier(int burnTime, int energyGen, ItemStack upgrade1, ItemStack upgrade2) {
		...
		if(totalSpeed > 0 || totalEfficiency > 0) {
			...
			if(totalSpeed != 0 && totalEfficiency != 0) {
		    	...
		    	this.totalEnergyGen = bothUpgradeEfficiency(energyGen, totalSpeed, totalEfficiency);
		    	this.totalBurnTime = bothUpgradeSpeed(burnTime, totalSpeed, totalEfficiency);
		    	this.onUpgradeChanged();

...
public class T4EnergyChuteNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T4EnergyChuteNetwork>	networks;
	final World world;
	
	public T4EnergyChuteNetworkList(final World world) {
	...
	public HashSet<T4EnergyChuteNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T4EnergyChuteNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T4EnergyChuteNetwork network = new T4EnergyChuteNetwork(this.world);
	...
	private void generateNetwork(final T4EnergyChuteNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT4EnergyChute)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT4EnergyChuteNetwork.T4_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T4_ENERGY_NETWORK_CHUTE.send(target, new ClientboundT4EnergyChuteNetworkPacket(syncTag));
				}
			}
		}
	}

...
public class T2EnergyCubeNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T2EnergyCubeNetwork>	networks;
	final World world;
	
	public T2EnergyCubeNetworkList(final World world) {
	...
	public HashSet<T2EnergyCubeNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T2EnergyCubeNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T2EnergyCubeNetwork network = new T2EnergyCubeNetwork(this.world);
	...
	private void generateNetwork(final T2EnergyCubeNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT2EnergyCube)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT2EnergyCubeNetwork.T2_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T2_ENERGY_NETWORK_CUBE.send(target, new ClientboundT2EnergyCubeNetworkPacket(syncTag));
				}
			}
		}
	}

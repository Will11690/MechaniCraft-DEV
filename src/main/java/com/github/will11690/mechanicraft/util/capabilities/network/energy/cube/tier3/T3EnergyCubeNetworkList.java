...
public class T3EnergyCubeNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T3EnergyCubeNetwork>	networks;
	final World world;
	
	public T3EnergyCubeNetworkList(final World world) {
	...
	public HashSet<T3EnergyCubeNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T3EnergyCubeNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T3EnergyCubeNetwork network = new T3EnergyCubeNetwork(this.world);
	...
	private void generateNetwork(final T3EnergyCubeNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT3EnergyCube)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT3EnergyCubeNetwork.T3_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T3_ENERGY_NETWORK_CUBE.send(target, new ClientboundT3EnergyCubeNetworkPacket(syncTag));
				}
			}
		}
	}

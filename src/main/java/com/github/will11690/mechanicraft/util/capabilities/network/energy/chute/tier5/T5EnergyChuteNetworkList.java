...
public class T5EnergyChuteNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T5EnergyChuteNetwork>	networks;
	final World world;
	
	public T5EnergyChuteNetworkList(final World world) {
	...
	public HashSet<T5EnergyChuteNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T5EnergyChuteNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T5EnergyChuteNetwork network = new T5EnergyChuteNetwork(this.world);
	...
	private void generateNetwork(final T5EnergyChuteNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT5EnergyChute)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT5EnergyChuteNetwork.T5_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T5_ENERGY_NETWORK_CHUTE.send(target, new ClientboundT5EnergyChuteNetworkPacket(syncTag));
				}
			}
		}
	}

...
public class T6EnergyChuteNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T6EnergyChuteNetwork>	networks;
	final World world;
	
	public T6EnergyChuteNetworkList(final World world) {
	...
	public HashSet<T6EnergyChuteNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T6EnergyChuteNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T6EnergyChuteNetwork network = new T6EnergyChuteNetwork(this.world);
	...
	private void generateNetwork(final T6EnergyChuteNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT6EnergyChute)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT6EnergyChuteNetwork.T6_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T6_ENERGY_NETWORK_CHUTE.send(target, new ClientboundT6EnergyChuteNetworkPacket(syncTag));
				}
			}
		}
	}

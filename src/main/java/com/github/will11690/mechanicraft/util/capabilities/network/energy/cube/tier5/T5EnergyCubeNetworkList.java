...
public class T5EnergyCubeNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T5EnergyCubeNetwork>	networks;
	final World world;
	
	public T5EnergyCubeNetworkList(final World world) {
	...
	public HashSet<T5EnergyCubeNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T5EnergyCubeNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T5EnergyCubeNetwork network = new T5EnergyCubeNetwork(this.world);
	...
	private void generateNetwork(final T5EnergyCubeNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT5EnergyCube)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT5EnergyCubeNetwork.T5_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T5_ENERGY_NETWORK_CUBE.send(target, new ClientboundT5EnergyCubeNetworkPacket(syncTag));
				}
			}
		}
	}

...
public class T6EnergyCubeNetworkList {

	private final HashSet<BlockPos>	connections;
	private final HashSet<T6EnergyCubeNetwork>	networks;
	final World world;
	
	public T6EnergyCubeNetworkList(final World world) {
	...
	public HashSet<T6EnergyCubeNetwork> getNetworks() {
	...
	public void tick() {
		...
		if(this.shouldDistributeEnergy()) {
			...
			for(final T6EnergyCubeNetwork network : this.networks) {
	...
	public void refreshNetworks() {
		...
		for(final BlockPos pos : this.getConnections()) {
			...
			if(!done.contains(pos)) {
				...
				final T6EnergyCubeNetwork network = new T6EnergyCubeNetwork(this.world);
	...
	private void generateNetwork(final T6EnergyCubeNetwork network, final BlockPos pos) {
		...
		if(!(tile instanceof TileEntityT6EnergyCube)) {
	...
	public void onChange(final BlockPos pos) {
		...
		if(!this.world.isClientSide) {
			...
			final INBT syncTag = CapabilityT6EnergyCubeNetwork.T6_NETWORK_LIST.writeNBT(this, null);
			...
			for(final PlayerEntity player : this.world.players()) {
				...
				if(player instanceof ServerPlayerEntity) {
					...
					PacketHandler.T6_ENERGY_NETWORK_CUBE.send(target, new ClientboundT6EnergyCubeNetworkPacket(syncTag));
				}
			}
		}
	}

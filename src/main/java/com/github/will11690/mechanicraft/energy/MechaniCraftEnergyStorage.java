...
public class MechaniCraftEnergyStorage extends EnergyStorage implements INBTSerializable<CompoundNBT> {
	...
	public int setEnergy(final int setEnergy) {
	...
	public void updateEnergyStorageNoUpgrades(int capacity) {
		...
		if(this.maxExtract <= 0 && baseCapacity > 0) {
			
			setMaxExtract(baseCapacity);
			onEnergyChanged();
		}
		
		if(this.maxReceive <= 0 && baseCapacity > 0) {
			
			setMaxReceive(baseCapacity);
			onEnergyChanged();
		}
	}
    ...
    public static int onlyCapacityUpgradeCapacity(double baseCapacity, int numUpgrades) {
    ...
    public static int onlyCapacityUpgradeTransfer(double baseTransfer, int numUpgrades) {
    	
        double transfer = baseTransfer;
        
        for (int i = 0; i < numUpgrades; i++) {
        	transfer -= (transfer * 0.05);
        }
        
        return Math.max(1, (int)transfer);
    }

    public static int onlyTransferUpgradeCapacity(double baseCapacity, int numUpgrades) {
    ...
    public static int onlyTransferUpgradeTransfer(double baseTransfer, int numUpgrades) {
	...
	public static int bothUpgradeCapacity(double baseCapacity, int numCapacityUpgrades, int numTransferUpgrades) {
		
    	double capacity = baseCapacity;
    	for (int i = 0; i < numCapacityUpgrades; i++) {
    		capacity += (capacity * 0.1);
        }
    	
    	for (int i = 0; i < numTransferUpgrades; i++) {
    		capacity -= (capacity * 0.05);
        }
	...
	public static int bothUpgradeTransfer(double baseTransfer, int numCapacityUpgrades, int numTransferUpgrades) {
	...
	public void oneUpgradeModifier(int capacity, int receive, int extract, ItemStack upgrade) {
		...
		if(totalCapacity > 0 || totalTransfer > 0) {
			...
			if(totalCapacity > 0 && totalTransfer == 0) {
				...
				this.setUpgradedCapacity(onlyCapacityUpgradeCapacity(baseCapacity, totalCapacity));
				
				if(baseExtract > 0) {
					
					this.setUpgradedExtract(onlyCapacityUpgradeTransfer(baseExtract, totalCapacity));
					
				} else {
					
					this.setUpgradedExtract(baseExtract);
				}
				
				if(baseReceive > 0) {
					
					this.setUpgradedReceive(onlyCapacityUpgradeTransfer(baseReceive, totalCapacity));
					
				} else {
					
					this.setUpgradedReceive(baseReceive);
				}
				...
				this.setUpgradedCapacity(onlyTransferUpgradeCapacity(baseCapacity, totalTransfer));
			
				if(baseExtract > 0) {
					
					this.setUpgradedExtract(onlyTransferUpgradeTransfer(baseExtract, totalTransfer));
					
				} else {
					
					this.setUpgradedExtract(baseReceive);
				}
		
				if(baseReceive > 0) {
					
					this.setUpgradedReceive(onlyTransferUpgradeTransfer(baseReceive, totalTransfer));
		...
		if(totalCapacity > 0 || totalTransfer > 0) {
			...
			if(totalCapacity != 0 && totalTransfer != 0) {
				...
				if(baseExtract > 0) {
					...
					this.setUpgradedExtract(bothUpgradeTransfer(baseExtract, totalCapacity, totalTransfer));
				...
				if(baseReceive > 0) {
					...
					this.setUpgradedReceive(bothUpgradeTransfer(baseReceive, totalCapacity, totalTransfer));
			...
			this.setUpgradedCapacity(bothUpgradeCapacity(baseCapacity, totalCapacity, totalTransfer));

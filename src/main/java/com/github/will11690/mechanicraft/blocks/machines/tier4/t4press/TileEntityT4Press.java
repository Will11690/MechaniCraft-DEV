...
public class TileEntityT4Press extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int pressingEnergy = ModConfigs.t4PressEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t4PressWorkTimeInt;
	...
	public TileEntityT4Press() {

		super(TileEntityHandler.TILE_ENTITY_T4_PRESS.get());
	...
	public void tick() {
		...
		if(!canPress() && this.level.getBlockState(this.worldPosition).getValue(T4Press.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4Press.LIT, Boolean.valueOf(false)));

		}

		if(canPress() && this.level.getBlockState(this.worldPosition).getValue(T4Press.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4Press.LIT, Boolean.valueOf(true)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.twoUpgradeModifier(WORK_TIME, pressingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT4Press(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t4_press");

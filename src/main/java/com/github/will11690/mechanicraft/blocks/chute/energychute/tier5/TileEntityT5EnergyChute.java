...
public class TileEntityT5EnergyChute extends TileEntity implements ITickableTileEntity {
	...
	private static int capacity = 20480;
	private static int transfer = 20480;
	
	public TileEntityT5EnergyChute() {
		
		super(TileEntityHandler.TILE_ENTITY_T5_ENERGY_CHUTE.get());
	}
	...
	private boolean canConnectBlock(Direction direction) {
		...
		if(tile != null && !(tile instanceof TileEntityT5EnergyChute) && tile.getCapability(CapabilityEnergy.ENERGY, direction.getOpposite()).isPresent()) {
	...
	public void onLoad() {
		...
		LazyOptional<T5EnergyChuteNetworkList> t5ChuteNetworkCap = level.getCapability(CapabilityT5EnergyChuteNetwork.T5_NETWORK_LIST);
		
		if(t5ChuteNetworkCap.isPresent()) {
			T5EnergyChuteNetworkList t5ChuteNetwork = t5ChuteNetworkCap.orElseThrow(IllegalStateException::new);
			t5ChuteNetwork.addConnection(this.worldPosition);
		}
		super.onLoad();
	}

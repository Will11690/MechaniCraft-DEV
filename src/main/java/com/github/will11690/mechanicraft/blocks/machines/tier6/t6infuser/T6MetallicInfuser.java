...
public class T6MetallicInfuser extends Block {
	
    public static final DirectionProperty FACING = HorizontalBlock.FACING;
    public static final BooleanProperty LIT = BlockStateProperties.LIT;

    public T6MetallicInfuser(Properties properties) {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        ...
        return TileEntityHandler.TILE_ENTITY_T6_METALLIC_INFUSER.get().create();
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult rayTrace) {
        ...
        if (!(te instanceof TileEntityT6MetallicInfuser))
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
    	 ...
    	 if(!state.is(newState.getBlock())) {
             ...
             if (tileEntity != null) {
            	 ((TileEntityT6MetallicInfuser) tileEntity).blockBeingBroken(true);
                 ...
                 cap.ifPresent(handler -> {
                	...
                	((TileEntityT6MetallicInfuser) tileEntity).blockBeingBroken(false);

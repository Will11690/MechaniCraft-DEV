...
public class TileEntityT6OreWasher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private static int fluid_capacity = ModConfigs.t6OreWasherTankCapacityInt;
    ...
    public TileEntityT6OreWasher() {
    	
        super(TileEntityHandler.TILE_ENTITY_T6_ORE_WASHER.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T6OreWasher.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6OreWasher.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T6OreWasher.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6OreWasher.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	upgradeSlotHandler.getStackInSlot(2).getItem().equals(ModItems.CAPACITY_UPGRADE.get()) || upgradeSlotHandler.getStackInSlot(3).getItem().equals(ModItems.CAPACITY_UPGRADE.get())) {
    		...
    		inputFluidTank.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t6_ore_washer");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT6OreWasher(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler));
    ...
    public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
        ...
        if (!this.remove && side != null) {
			...
			if(cap == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6OreWasher.FACING) == Direction.NORTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6OreWasher.FACING) == Direction.SOUTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6OreWasher.FACING) == Direction.EAST) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6OreWasher.FACING) == Direction.WEST) {

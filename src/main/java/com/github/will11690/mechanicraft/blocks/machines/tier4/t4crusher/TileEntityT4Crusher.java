package com.github.will11690.mechanicraft.blocks.machines.tier4.t4crusher;
...
public class TileEntityT4Crusher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int crushingEnergy = ModConfigs.t4CrusherEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t4CrusherWorkTimeInt;
	...
	public TileEntityT4Crusher() {

		super(TileEntityHandler.TILE_ENTITY_T4_CRUSHER.get());
	...
	public void tick() {
		...
		if(!canCrush() && this.level.getBlockState(this.worldPosition).getValue(T4Crusher.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4Crusher.LIT, Boolean.valueOf(false)));
		...
		if(canCrush() && this.level.getBlockState(this.worldPosition).getValue(T4Crusher.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4Crusher.LIT, Boolean.valueOf(true)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.twoUpgradeModifier(WORK_TIME, crushingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT4Crusher(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t4_crusher");

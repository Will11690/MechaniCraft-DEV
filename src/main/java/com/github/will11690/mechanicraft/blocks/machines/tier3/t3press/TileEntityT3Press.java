...
public class TileEntityT3Press extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public TileEntityT3Press() {

		super(TileEntityHandler.TILE_ENTITY_T3_PRESS.get());
	...
	public void tick() {
		...
		if(!canPress() && this.level.getBlockState(this.worldPosition).getValue(T3Press.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T3Press.LIT, Boolean.valueOf(false)));

		}

		if(canPress() && this.level.getBlockState(this.worldPosition).getValue(T3Press.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T3Press.LIT, Boolean.valueOf(true)));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT3Press(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t3_press");

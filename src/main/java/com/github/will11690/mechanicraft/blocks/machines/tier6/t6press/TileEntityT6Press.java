...
public class TileEntityT6Press extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int pressingEnergy = ModConfigs.t6PressEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t6PressWorkTimeInt;
		
	private static int capacity = ModConfigs.t6PressCapacityInt;
	private static int receive = ModConfigs.t6PressReceiveInt;
	...
	public TileEntityT6Press() {

		super(TileEntityHandler.TILE_ENTITY_T6_PRESS.get());
	...
	private ItemStackHandler createInputWrapper(ItemStackHandler inputSlot) {
		...
		return new NonExtractableStackHandler(this.inputSlotHandler) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				...
				if(slot == 0 && isRecipeInput6(level, stack)) {
	...
	public void tick() {
		...
		if(!canPress() && this.level.getBlockState(this.worldPosition).getValue(T6Press.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6Press.LIT, Boolean.valueOf(false)));

		}

		if(canPress() && this.level.getBlockState(this.worldPosition).getValue(T6Press.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6Press.LIT, Boolean.valueOf(true)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.fourUpgradeModifier(WORK_TIME, pressingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT6Press(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t6_press");

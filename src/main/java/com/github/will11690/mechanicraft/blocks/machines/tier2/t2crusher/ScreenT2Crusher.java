...
public class ScreenT2Crusher extends ContainerScreen<ContainerT2Crusher> {
	
	private static final ResourceLocation TEXTURE = new ResourceLocation(Reference.MOD_ID + ":textures/gui/crusher/t2crusher.png");
	...
	public ScreenT2Crusher(ContainerT2Crusher container, PlayerInventory playerInventory, ITextComponent title) {
	...
	public void render(MatrixStack matrixStack, int x, int y, float partialTicks) {
		...
		if(Screen.hasShiftDown()) {
			...
			if (x > (getGuiLeft() + INFO_ENERGY_BAR_TO_X) && x < (getGuiLeft() + INFO_ENERGY_BAR_TO_X) + INFO_ENERGY_BAR_WIDTH && y > (getGuiTop() + INFO_ENERGY_BAR_TO_Y) && y < (getGuiTop() + INFO_ENERGY_BAR_TO_Y) + INFO_ENERGY_BAR_HEIGHT) {
				...
				new TranslationTextComponent("com.github.will11690.mechanicraft.screen.t2_crusher.energy", this.menu.getEnergyStored(), this.menu.getEnergyCapacity()))), x, y);
			...
			if (x > (getGuiLeft() + INFO_PROGRESS_METER_TO_X) && x < (getGuiLeft() + INFO_PROGRESS_METER_TO_X) + INFO_PROGRESS_METER_WIDTH && y > (getGuiTop() + INFO_PROGRESS_METER_TO_Y) && y < (getGuiTop() + INFO_PROGRESS_METER_TO_Y) + INFO_PROGRESS_METER_HEIGHT) {
				...
				this.renderTooltip(matrixStack, LanguageMap.getInstance().getVisualOrder(Arrays.asList(
	            new TranslationTextComponent("com.github.will11690.mechanicraft.screen.t2_crusher.cook_progress", this.menu.getProgress(), this.menu.getMaxProgress()))), x, y);
			...
			if (x > (getGuiLeft() + INFO_ENERGY_BAR_TO_X) && x < (getGuiLeft() + INFO_ENERGY_BAR_TO_X) + INFO_ENERGY_BAR_WIDTH && y > (getGuiTop() + INFO_ENERGY_BAR_TO_Y) && y < (getGuiTop() + INFO_ENERGY_BAR_TO_Y) + INFO_ENERGY_BAR_HEIGHT) {
				...
				this.renderTooltip(matrixStack, LanguageMap.getInstance().getVisualOrder(Arrays.asList(
	        	new TranslationTextComponent("com.github.will11690.mechanicraft.screen.t2_crusher.energy", Utils.withSuffix(this.menu.getEnergyStored()), Utils.withSuffix(this.menu.getEnergyCapacity())),
			...
			if (x > (getGuiLeft() + INFO_PROGRESS_METER_TO_X) && x < (getGuiLeft() + INFO_PROGRESS_METER_TO_X) + INFO_PROGRESS_METER_WIDTH && y > (getGuiTop() + INFO_PROGRESS_METER_TO_Y) && y < (getGuiTop() + INFO_PROGRESS_METER_TO_Y) + INFO_PROGRESS_METER_HEIGHT) {
				...
				this.renderTooltip(matrixStack, LanguageMap.getInstance().getVisualOrder(Arrays.asList(
	            new TranslationTextComponent("com.github.will11690.mechanicraft.screen.t2_crusher.cook_progress", Utils.withSuffix(this.menu.getProgress()), Utils.withSuffix(this.menu.getMaxProgress())),
	            new TranslationTextComponent("com.github.will11690.mechanicraft.screen.gui_details"))), x, y);

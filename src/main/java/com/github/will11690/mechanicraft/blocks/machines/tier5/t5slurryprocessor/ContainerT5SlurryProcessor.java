...
public class ContainerT5SlurryProcessor extends Container {
    ...
    public ContainerT5SlurryProcessor(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
    	this((TileEntityT5SlurryProcessor) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(5));
    	
    }
    
    public ContainerT5SlurryProcessor(@Nullable TileEntityT5SlurryProcessor tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T5_SLURRY_PROCESSOR.get(), id);
        ...
        this.addSlot(new SlotEnergyItem(this.handler, 3, 8, 55));

        this.addSlot(new SlotItemHandler(this.handler, 4, 103, 32) {
    ...
    private IFluidHandler getFluid1() {
    	//INPUT 1
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T5SlurryProcessor.FACING).getClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid2() {
    	//INPUT 2
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T5SlurryProcessor.FACING).getCounterClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid3() {
    	//OUTPUT
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T5SlurryProcessor.FACING).getOpposite()).orElse(null);
    }

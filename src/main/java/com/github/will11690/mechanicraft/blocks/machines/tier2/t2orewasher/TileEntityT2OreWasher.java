...
public class TileEntityT2OreWasher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int washingEnergy = ModConfigs.t2OreWasherEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t2OreWasherWorkTimeInt;
		
	private static int capacity = ModConfigs.t2OreWasherEnergyCapacityInt;
	private static int receive = ModConfigs.t2OreWasherReceiveInt;
	private static int fluid_capacity = ModConfigs.t2OreWasherTankCapacityInt;
    ...
    public TileEntityT2OreWasher() {
    	
        super(TileEntityHandler.TILE_ENTITY_T2_ORE_WASHER.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T2OreWasher.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2OreWasher.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T2OreWasher.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2OreWasher.LIT, Boolean.valueOf(false)));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t2_ore_washer");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT2OreWasher(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler));

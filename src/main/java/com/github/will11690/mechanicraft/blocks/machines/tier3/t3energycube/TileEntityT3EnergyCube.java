...
public class TileEntityT3EnergyCube extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    private static int capacity = ModConfigs.t3EnergyCubeCapacityInt;
    private static int transfer = ModConfigs.t3EnergyCubeTransferInt;
	...
	public void onLoad() {
		...
		LazyOptional<T3EnergyCubeNetworkList> t3CubeNetworkCap = level.getCapability(CapabilityT3EnergyCubeNetwork.T3_NETWORK_LIST);
		
		if(t3CubeNetworkCap.isPresent()) {
			T3EnergyCubeNetworkList t3CubeNetwork = t3CubeNetworkCap.orElseThrow(IllegalStateException::new);
			t3CubeNetwork.addConnection(this.worldPosition);
		}
		super.onLoad();
	}
	...
	private void sendPower() {
        ...
        if(energy.get() > 0) {
            ...
            for(Direction direction : Direction.values()) {
                ...
                if(te != null && !(te instanceof TileEntityT3EnergyCube)) {
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        assert level != null;
        return new ContainerT3EnergyCube(this, id, playerInventory, new CombinedInvWrapper(chargeSlotsHandler, upgradeSlotHandler));
    }

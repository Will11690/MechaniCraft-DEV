...
public class ContainerT2SlurryProcessor extends Container {
    ...
    public ContainerT2SlurryProcessor(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
    	this((TileEntityT2SlurryProcessor) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(3));
    	
    }
    
    public ContainerT2SlurryProcessor(@Nullable TileEntityT2SlurryProcessor tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T2_SLURRY_PROCESSOR.get(), id);
    ...
    private IFluidHandler getFluid1() {
    	//INPUT 1
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T2SlurryProcessor.FACING).getClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid2() {
    	//INPUT 2
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T2SlurryProcessor.FACING).getCounterClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid3() {
    	//OUTPUT
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T2SlurryProcessor.FACING).getOpposite()).orElse(null);
    }

...
public class TileEntityT4SlurryProcessor extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int processingEnergy = ModConfigs.t4SlurryProcessorEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t4SlurryProcessorWorkTimeInt;
    ...
    public TileEntityT4SlurryProcessor() {
    	
        super(TileEntityHandler.TILE_ENTITY_T4_SLURRY_PROCESSOR.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T4SlurryProcessor.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4SlurryProcessor.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T4SlurryProcessor.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4SlurryProcessor.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.twoUpgradeModifier(WORK_TIME, processingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t4_slurry_processor");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT4SlurryProcessor(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, outputSlotHandler));

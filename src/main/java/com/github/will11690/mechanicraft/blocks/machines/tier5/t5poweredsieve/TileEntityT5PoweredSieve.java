package com.github.will11690.mechanicraft.blocks.machines.tier5.t5poweredsieve;
...
public class TileEntityT5PoweredSieve extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int sievingEnergy = ModConfigs.t5PoweredSieveEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t5PoweredSieveWorkTimeInt;
		
	private static int capacity = ModConfigs.t5PoweredSieveCapacityInt;
	private static int receive = ModConfigs.t5PoweredSieveReceiveInt;
    ...
    public TileEntityT5PoweredSieve() {
    	
        super(TileEntityHandler.TILE_ENTITY_T5_POWERED_SIEVE.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T5PoweredSieve.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T5PoweredSieve.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T5PoweredSieve.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T5PoweredSieve.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.threeUpgradeModifier(WORK_TIME, sievingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t5_powered_sieve");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT5PoweredSieve(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler1, inputSlotHandler2, outputSlotHandler));

package com.github.will11690.mechanicraft.blocks.machines.tier6.t6poweredsieve;
...
public class TileEntityT6PoweredSieve extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int sievingEnergy = ModConfigs.t6PoweredSieveEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t6PoweredSieveWorkTimeInt;
		
	private static int capacity = ModConfigs.t6PoweredSieveCapacityInt;
	private static int receive = ModConfigs.t6PoweredSieveReceiveInt;
    ...
    public TileEntityT6PoweredSieve() {
    	
        super(TileEntityHandler.TILE_ENTITY_T6_POWERED_SIEVE.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T6PoweredSieve.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6PoweredSieve.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T6PoweredSieve.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6PoweredSieve.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.fourUpgradeModifier(WORK_TIME, sievingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t6_powered_sieve");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT6PoweredSieve(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler1, inputSlotHandler2, outputSlotHandler));

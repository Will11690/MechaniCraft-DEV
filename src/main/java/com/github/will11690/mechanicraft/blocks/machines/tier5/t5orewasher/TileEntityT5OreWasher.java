...
public class TileEntityT5OreWasher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int washingEnergy = ModConfigs.t5OreWasherEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t5OreWasherWorkTimeInt;
    ...
    public TileEntityT5OreWasher() {
    	
        super(TileEntityHandler.TILE_ENTITY_T5_ORE_WASHER.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T5OreWasher.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T5OreWasher.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T5OreWasher.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T5OreWasher.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.threeUpgradeModifier(WORK_TIME, washingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2));
    	...
    	if(upgradeSlotHandler.getStackInSlot(0).getItem().equals(ModItems.CAPACITY_UPGRADE.get()) || upgradeSlotHandler.getStackInSlot(1).getItem().equals(ModItems.CAPACITY_UPGRADE.get()) ||
    		...
    		inputFluidTank.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), ItemStack.EMPTY);
    		outputFluidTank.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), ItemStack.EMPTY);
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t5_ore_washer");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT5OreWasher(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler));
    ...
    public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
        ...
        if (!this.remove && side != null) {
			...
			if(cap == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T5OreWasher.FACING) == Direction.NORTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T5OreWasher.FACING) == Direction.SOUTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T5OreWasher.FACING) == Direction.EAST) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T5OreWasher.FACING) == Direction.WEST) {

package com.github.will11690.mechanicraft.blocks.fluidtank.basicfluidtank;
...
public class TileEntityBasicFluidTank extends TileEntity implements ITickableTileEntity {
    ...
    public static final int capacity = ModConfigs.basicTankCapacityInt;

    public TileEntityBasicFluidTank() {
		
		super(TileEntityHandler.TILE_ENTITY_BASIC_FLUID_TANK.get());
	...
	public void tick() {
		...
		if(this.getBlockState().getValue(BasicFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(true)) {
				...
				if(neighborEntity.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY).isPresent()) {
					...
					if(this.fluidHandler.isPresent()) {
						...
						if(fluidTank.getFluidInTank(0).getAmount() > 0) {
							...
							FluidUtil.tryFluidTransfer(neighborFluidHandler, outputFluidHandler, Math.min(fluidTank.getFluidInTank(0).getAmount(), FluidAttributes.BUCKET_VOLUME), true);
						}
					}
				}
			}
	...
	void setOutputBottomTrue() {
		...
		if(this.getBlockState().getValue(BasicFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(false)) {
			
			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(BasicFluidTank.OUTPUT_BOTTOM, Boolean.valueOf(true)));
		}
	}
	...
	void setOutputBottomFalse() {
		...
		if(this.getBlockState().getValue(BasicFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(true)) {
			...
			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(BasicFluidTank.OUTPUT_BOTTOM, Boolean.valueOf(false)));
		}
	}

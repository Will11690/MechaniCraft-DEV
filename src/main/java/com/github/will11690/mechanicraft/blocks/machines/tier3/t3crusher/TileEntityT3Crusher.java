package com.github.will11690.mechanicraft.blocks.machines.tier3.t3crusher;
...
public class TileEntityT3Crusher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int crushingEnergy = ModConfigs.t3CrusherEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t3CrusherWorkTimeInt;
		
	private static int capacity = ModConfigs.t3CrusherCapacityInt;
	private static int receive = ModConfigs.t3CrusherReceiveInt;
	...
	public TileEntityT3Crusher() {

		super(TileEntityHandler.TILE_ENTITY_T3_CRUSHER.get());
	...
	public void tick() {
		...
		if(!canCrush() && this.level.getBlockState(this.worldPosition).getValue(T3Crusher.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T3Crusher.LIT, Boolean.valueOf(false)));
		...
		if(canCrush() && this.level.getBlockState(this.worldPosition).getValue(T3Crusher.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T3Crusher.LIT, Boolean.valueOf(true)));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT3Crusher(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t3_crusher");

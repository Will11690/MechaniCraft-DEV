...
public class TileEntityT4MetallicInfuser extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int infusingEnergy = ModConfigs.t4InfuserEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t4InfuserWorkTimeInt;
    ...
    public TileEntityT4MetallicInfuser() {
    	
        super(TileEntityHandler.TILE_ENTITY_T4_METALLIC_INFUSER.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T4MetallicInfuser.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4MetallicInfuser.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T4MetallicInfuser.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4MetallicInfuser.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.twoUpgradeModifier(WORK_TIME, infusingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t4_metallic_infuser");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT4MetallicInfuser(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler1, inputSlotHandler2, outputSlotHandler));

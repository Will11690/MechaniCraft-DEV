package com.github.will11690.mechanicraft.blocks.machines.tier2.t2poweredsieve;
...
public class TileEntityT2PoweredSieve extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int sievingEnergy = ModConfigs.t2PoweredSieveEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t2PoweredSieveWorkTimeInt;
		
	private static int capacity = ModConfigs.t2PoweredSieveCapacityInt;
	private static int receive = ModConfigs.t2PoweredSieveReceiveInt;
    ...
    public TileEntityT2PoweredSieve() {
    	
        super(TileEntityHandler.TILE_ENTITY_T2_POWERED_SIEVE.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T2PoweredSieve.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2PoweredSieve.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T2PoweredSieve.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2PoweredSieve.LIT, Boolean.valueOf(false)));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t2_powered_sieve");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT2PoweredSieve(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler1, inputSlotHandler2, outputSlotHandler));

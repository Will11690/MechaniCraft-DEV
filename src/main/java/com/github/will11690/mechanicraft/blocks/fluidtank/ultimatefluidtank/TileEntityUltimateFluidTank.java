package com.github.will11690.mechanicraft.blocks.fluidtank.ultimatefluidtank;
...
public class TileEntityUltimateFluidTank extends TileEntity implements ITickableTileEntity {
    ...
    public static final int capacity = ModConfigs.ultimateTankCapacityInt;

    public TileEntityUltimateFluidTank() {
		
		super(TileEntityHandler.TILE_ENTITY_ULTIMATE_FLUID_TANK.get());
	...
	public void tick() {
		...
		if(this.getBlockState().getValue(UltimateFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(true)) {
				...
				if(neighborEntity.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY).isPresent()) {
					...
					if(this.fluidHandler.isPresent()) {
						...
						if(fluidTank.getFluidInTank(0).getAmount() > 0) {
							...
							FluidUtil.tryFluidTransfer(neighborFluidHandler, outputFluidHandler, Math.min(fluidTank.getFluidInTank(0).getAmount(), FluidAttributes.BUCKET_VOLUME * 32), true);
						}
					}
				}
			}
	...
	void setOutputBottomTrue() {
		...
		if(this.getBlockState().getValue(UltimateFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(false)) {
			
			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(UltimateFluidTank.OUTPUT_BOTTOM, Boolean.valueOf(true)));
		}
	}
	...
	void setOutputBottomFalse() {
		...
		if(this.getBlockState().getValue(UltimateFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(true)) {
			...
			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(UltimateFluidTank.OUTPUT_BOTTOM, Boolean.valueOf(false)));
		}
	}
    ...
    public void load(BlockState state, CompoundNBT tags) {
        ...
        if(tags.contains("ultimateFluidTank", Constants.NBT.TAG_COMPOUND)) {
			
            CompoundNBT fluidTankTag = tags.getCompound("ultimateFluidTank");
    ...
    public CompoundNBT save(CompoundNBT tags) {
        ...
        tags.put("ultimateFluidTank", fluidTankTag);
    ...
    public void onDataPacket(NetworkManager net, SUpdateTileEntityPacket packet) {
    	...
    	fluidTank.readFromNBT(packet.getTag().getCompound("ultimateFluidTank"));

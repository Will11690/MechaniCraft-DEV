...
public class ContainerT4OreWasher extends Container {
    ...
    public ContainerT4OreWasher(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
    	this((TileEntityT4OreWasher) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(4));
    	
    }
    
    public ContainerT4OreWasher(@Nullable TileEntityT4OreWasher tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T4_ORE_WASHER.get(), id);
        ...
        this.addSlot(new SlotUpgrade(this.handler, 1, 155, 44) {
        ...
        this.addSlot(new SlotItemHandler(this.handler, 3, 71, 31) {
        	
            @Override
            public boolean mayPlace(ItemStack stack) {
            	
            	if(TileEntityT4OreWasher.isRecipeInputStack(tile.getLevel(), stack)) {
    ...
    private IFluidHandler getFluid2() {
    	...
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T4OreWasher.FACING).getOpposite()).orElse(null);
    }

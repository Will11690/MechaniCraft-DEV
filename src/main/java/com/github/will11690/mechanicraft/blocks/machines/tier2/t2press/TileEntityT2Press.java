...
public class TileEntityT2Press extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int pressingEnergy = ModConfigs.t2PressEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t2PressWorkTimeInt;
	...
	public TileEntityT2Press() {

		super(TileEntityHandler.TILE_ENTITY_T2_PRESS.get());
	...
	private ItemStackHandler createInputWrapper(ItemStackHandler inputSlot) {
		...
		return new NonExtractableStackHandler(this.inputSlotHandler) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				...
				if(slot == 0 && isRecipeInput2(level, stack)) {

					return true;

				}
				
				if(slot == 1 && isRecipeInput1(level, stack)) {
	...
	public void tick() {
		...
		if(!canPress() && this.level.getBlockState(this.worldPosition).getValue(T2Press.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2Press.LIT, Boolean.valueOf(false)));

		}

		if(canPress() && this.level.getBlockState(this.worldPosition).getValue(T2Press.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2Press.LIT, Boolean.valueOf(true)));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT2Press(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t2_press");

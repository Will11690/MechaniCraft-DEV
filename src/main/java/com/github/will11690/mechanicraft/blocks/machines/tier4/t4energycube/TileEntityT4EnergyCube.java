...
public class TileEntityT4EnergyCube extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    private static int capacity = ModConfigs.t4EnergyCubeCapacityInt;
    private static int transfer = ModConfigs.t4EnergyCubeTransferInt;
    ...
    public TileEntityT4EnergyCube() {
    	
        super(TileEntityHandler.TILE_ENTITY_T4_ENERGY_CUBE.get());
	...
	public void onLoad() {
		...
		LazyOptional<T4EnergyCubeNetworkList> t4CubeNetworkCap = level.getCapability(CapabilityT4EnergyCubeNetwork.T4_NETWORK_LIST);
		
		if(t4CubeNetworkCap.isPresent()) {
			T4EnergyCubeNetworkList t4CubeNetwork = t4CubeNetworkCap.orElseThrow(IllegalStateException::new);
			t4CubeNetwork.addConnection(this.worldPosition);
		}
		super.onLoad();
	}
    ...
    private void setUpgradeModifiers() {
    	...
    	if(energy.isPresent()) {
    		...
    		energyStorage.twoUpgradeModifier(capacity, transfer, transfer, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1));
	...
	private void sendPower() {
        ...
        if(energy.get() > 0) {
            ...
            for(Direction direction : Direction.values()) {
                ...
                if(te != null && !(te instanceof TileEntityT4EnergyCube)) {
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        assert level != null;
        return new ContainerT4EnergyCube(this, id, playerInventory, new CombinedInvWrapper(chargeSlotsHandler, upgradeSlotHandler));
    }
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t4_energy_cube");

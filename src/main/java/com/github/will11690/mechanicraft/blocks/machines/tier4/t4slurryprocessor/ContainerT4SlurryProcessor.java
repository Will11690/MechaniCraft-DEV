...
public class ContainerT4SlurryProcessor extends Container {
	
	private final IItemHandler playerInventory;
	private PlayerEntity player;
    private IItemHandler handler;
    private IIntArray fields;
    private TileEntityT4SlurryProcessor tile;
    ...
    public ContainerT4SlurryProcessor(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
    	this((TileEntityT4SlurryProcessor) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(4));
    	
    }
    
    public ContainerT4SlurryProcessor(@Nullable TileEntityT4SlurryProcessor tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T4_SLURRY_PROCESSOR.get(), id);
        ...
        this.addSlot(new SlotUpgrade(this.handler, 1, 152, 44) {
        ...
        this.addSlot(new SlotItemHandler(this.handler, 3, 103, 32) {
    ...
    private IFluidHandler getFluid1() {
    	//INPUT 1
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T4SlurryProcessor.FACING).getClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid2() {
    	//INPUT 2
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T4SlurryProcessor.FACING).getCounterClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid3() {
    	//OUTPUT
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T4SlurryProcessor.FACING).getOpposite()).orElse(null);
    }

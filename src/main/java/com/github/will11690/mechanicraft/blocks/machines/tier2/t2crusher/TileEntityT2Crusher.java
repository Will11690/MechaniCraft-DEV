package com.github.will11690.mechanicraft.blocks.machines.tier2.t2crusher;
...
public class TileEntityT2Crusher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int crushingEnergy = ModConfigs.t2CrusherEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t2CrusherWorkTimeInt;
		
	private static int capacity = ModConfigs.t2CrusherCapacityInt;
	private static int receive = ModConfigs.t2CrusherReceiveInt;
	...
	public TileEntityT2Crusher() {

		super(TileEntityHandler.TILE_ENTITY_T2_CRUSHER.get());
	}
	...
	public void tick() {
		...
		if(!canCrush() && this.level.getBlockState(this.worldPosition).getValue(T2Crusher.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2Crusher.LIT, Boolean.valueOf(false)));
		...
		if(canCrush() && this.level.getBlockState(this.worldPosition).getValue(T2Crusher.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2Crusher.LIT, Boolean.valueOf(true)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.oneUpgradeModifier(WORK_TIME, crushingEnergy, upgradeSlotHandler.getStackInSlot(0));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT2Crusher(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	}
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t2_crusher");
	}
}
...
public class TileEntityLineMiner extends TileEntity implements ITickableTileEntity {
	...
	public void tick() {
		...
		if(energyStorage.getCapacity() == 0 && energyStorage.getCapacity() != powerToConsume() && powerConsumed == false) {
			
			capacity = powerToConsume();
			energyStorage.setCapacity(powerToConsume());
		}
		
		if(energyStorage.getCapacity() > 0 && powerConsumed == true) {

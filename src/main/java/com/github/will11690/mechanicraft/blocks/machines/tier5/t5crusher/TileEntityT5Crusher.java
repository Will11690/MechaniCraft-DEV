package com.github.will11690.mechanicraft.blocks.machines.tier5.t5crusher;
...
public class TileEntityT5Crusher extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int crushingEnergy = ModConfigs.t5CrusherEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t5CrusherWorkTimeInt;
		
	private static int capacity = ModConfigs.t5CrusherCapacityInt;
	private static int receive = ModConfigs.t5CrusherReceiveInt;
	...
	public TileEntityT5Crusher() {

		super(TileEntityHandler.TILE_ENTITY_T5_CRUSHER.get());
	...
	public void tick() {
		...
		if(!canCrush() && this.level.getBlockState(this.worldPosition).getValue(T5Crusher.LIT) == true) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T5Crusher.LIT, Boolean.valueOf(false)));
		...
		if(canCrush() && this.level.getBlockState(this.worldPosition).getValue(T5Crusher.LIT) == false) {

			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T5Crusher.LIT, Boolean.valueOf(true)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.threeUpgradeModifier(WORK_TIME, crushingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2));
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT5Crusher(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, upgradeSlotHandler, outputSlotHandler));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t5_crusher");

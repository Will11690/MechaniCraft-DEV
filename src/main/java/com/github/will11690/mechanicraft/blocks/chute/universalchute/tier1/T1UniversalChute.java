...
public class T1UniversalChute extends Block {
	...
	public T1UniversalChute(Properties properties) {
	...
	public static boolean canChuteConnectChute(IBlockReader world, BlockPos pos, Direction facing) {
		...
		if(neighborBlock == ModBlocks.T1_UNIVERSAL_CHUTE.get()) {
	...
	public static boolean canChuteConnectBlock(IBlockReader world, BlockPos pos, Direction facing) {
		...
		if(neighborBlock.hasTileEntity(neighborState) && (neighborEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, facing).isPresent() || 
		neighborEntity.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, facing).isPresent() || 
		neighborEntity.getCapability(CapabilityEnergy.ENERGY, facing).isPresent()) && neighborBlock != ModBlocks.T1_UNIVERSAL_CHUTE.get()) {
	...
	return TileEntityHandler.TILE_ENTITY_T1_UNIVERSAL_CHUTE.get().create();

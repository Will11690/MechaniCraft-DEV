...
public class TileEntityAdvancedFurnace extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private final LazyOptional<IItemHandler> outputSlot1  = LazyOptional.of(() -> outputSlotHandler1);
	
	private final LazyOptional<IItemHandler> inputSlotWrapper2  = LazyOptional.of(() -> inputSlotWrapperHandler2);
	private final LazyOptional<IItemHandler> outputSlot2  = LazyOptional.of(() -> outputSlotHandler2);
	...
	private int smeltingEnergy = ModConfigs.advancedFurnaceEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.advancedFurnaceWorkTimeInt;
	
	private static int capacity = ModConfigs.advancedFurnaceCapacityInt;
	private static int receive = ModConfigs.advancedFurnaceReceiveInt;
    	...
    	return new ItemStackHandler(2) {
    		...
    		public ItemStack extractItem(int slot, int amount, boolean simulate) {
    			...
    			if(stacks.get(slot).getItem().equals(ModItems.SPEED_UPGRADE.get()) || stacks.get(slot).getItem().equals(ModItems.EFFICIENCY_UPGRADE.get())) {
    				...
    				if(upgradeHandler.canExtractFromSlot(progress1 + progress2) != true) {

package com.github.will11690.mechanicraft.blocks.machines.tier4.t4poweredsieve;
...
public class TileEntityT4PoweredSieve extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int sievingEnergy = ModConfigs.t4PoweredSieveEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t4PoweredSieveWorkTimeInt;
    ...
    public TileEntityT4PoweredSieve() {
    	
        super(TileEntityHandler.TILE_ENTITY_T4_POWERED_SIEVE.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T4PoweredSieve.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4PoweredSieve.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T4PoweredSieve.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T4PoweredSieve.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.twoUpgradeModifier(WORK_TIME, sievingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t4_powered_sieve");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT4PoweredSieve(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, inputSlotHandler1, inputSlotHandler2, outputSlotHandler));

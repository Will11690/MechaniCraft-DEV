...
public class TileEntityT5EnergyCube extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public void onLoad() {
		...
		LazyOptional<T5EnergyCubeNetworkList> t5CubeNetworkCap = level.getCapability(CapabilityT5EnergyCubeNetwork.T5_NETWORK_LIST);
		
		if(t5CubeNetworkCap.isPresent()) {
			T5EnergyCubeNetworkList t5CubeNetwork = t5CubeNetworkCap.orElseThrow(IllegalStateException::new);
			t5CubeNetwork.addConnection(this.worldPosition);
		}
		super.onLoad();
	}
	...
	private void sendPower() {
        ...
        if(energy.get() > 0) {
            ...
            for(Direction direction : Direction.values()) {
                ...
                if(te != null && !(te instanceof TileEntityT5EnergyCube)) {

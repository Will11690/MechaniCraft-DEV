...
public class TileEntityT2SlurryProcessor extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    public TileEntityT2SlurryProcessor() {
    	
        super(TileEntityHandler.TILE_ENTITY_T2_SLURRY_PROCESSOR.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T2SlurryProcessor.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2SlurryProcessor.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T2SlurryProcessor.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T2SlurryProcessor.LIT, Boolean.valueOf(false)));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t2_slurry_processor");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT2SlurryProcessor(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, outputSlotHandler));

...
public class ScreenT4EnergyCube extends ContainerScreen<ContainerT4EnergyCube> {
	
	private static final ResourceLocation TEXTURE = new ResourceLocation(Reference.MOD_ID + ":textures/gui/energy_cube/t4energy_cube.png");
    ...
    public ScreenT4EnergyCube(ContainerT4EnergyCube container, PlayerInventory playerInventory, ITextComponent title) {
    ...
    public void render(MatrixStack matrixStack, int x, int y, float partialTicks) {
        ...
        if(Screen.hasShiftDown()) {
        	...
        	if (x > (getGuiLeft() + INFO_ENERGY_BAR_TO_X) && x < (getGuiLeft() + INFO_ENERGY_BAR_TO_X) + INFO_ENERGY_BAR_WIDTH && y > (getGuiTop() + INFO_ENERGY_BAR_TO_Y) && y < (getGuiTop() + INFO_ENERGY_BAR_TO_Y) + INFO_ENERGY_BAR_HEIGHT) {
                ...
                new TranslationTextComponent("com.github.will11690.mechanicraft.screen.t4_energy_cube.energy", this.menu.getEnergyStored(), this.menu.getEnergyCapacity()))), x, y);
        	...
        	if (x > (getGuiLeft() + INFO_ENERGY_BAR_TO_X) && x < (getGuiLeft() + INFO_ENERGY_BAR_TO_X) + INFO_ENERGY_BAR_WIDTH && y > (getGuiTop() + INFO_ENERGY_BAR_TO_Y) && y < (getGuiTop() + INFO_ENERGY_BAR_TO_Y) + INFO_ENERGY_BAR_HEIGHT) {
        		...
        		this.renderTooltip(matrixStack, LanguageMap.getInstance().getVisualOrder(Arrays.asList(
        		new TranslationTextComponent("com.github.will11690.mechanicraft.screen.t4_energy_cube.energy", Utils.withSuffix(this.menu.getEnergyStored()), Utils.withSuffix(this.menu.getEnergyCapacity())),

...
public class ContainerT3SlurryProcessor extends Container {
	
	private final IItemHandler playerInventory;
	private PlayerEntity player;
    private IItemHandler handler;
    private IIntArray fields;
    private TileEntityT3SlurryProcessor tile;
    ...
    public ContainerT3SlurryProcessor(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
    	this((TileEntityT3SlurryProcessor) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(3));
    	
    }
    
    public ContainerT3SlurryProcessor(@Nullable TileEntityT3SlurryProcessor tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T3_SLURRY_PROCESSOR.get(), id);
        ...
        this.addSlot(new SlotUpgrade(this.handler, 0, 152, 24) {
        ...
        this.addSlot(new SlotEnergyItem(this.handler, 1, 8, 55));

        this.addSlot(new SlotItemHandler(this.handler, 2, 103, 32) {
    ...
    private IFluidHandler getFluid1() {
    	//INPUT 1
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T3SlurryProcessor.FACING).getClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid2() {
    	//INPUT 2
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T3SlurryProcessor.FACING).getCounterClockWise()).orElse(null);
    }
    
    private IFluidHandler getFluid3() {
    	//OUTPUT
    	return tile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, tile.getBlockState().getValue(T3SlurryProcessor.FACING).getOpposite()).orElse(null);
    }

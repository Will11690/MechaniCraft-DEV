...
public class TileEntityT3SlurryProcessor extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private final ItemStackHandler chargeSlotHandler = createCharge();
	...
	private int processingEnergy = ModConfigs.t3SlurryProcessorEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t3SlurryProcessorWorkTimeInt;
		
	private static int capacity = ModConfigs.t3SlurryProcessorEnergyCapacityInt;
	private static int receive = ModConfigs.t3SlurryProcessorReceiveInt;
	private static int fluid_capacity = ModConfigs.t3SlurryProcessorTankCapacityInt;
    ...
    public TileEntityT3SlurryProcessor() {
    	
        super(TileEntityHandler.TILE_ENTITY_T3_SLURRY_PROCESSOR.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T3SlurryProcessor.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T3SlurryProcessor.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T3SlurryProcessor.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T3SlurryProcessor.LIT, Boolean.valueOf(false)));
	...
	private void setUpgradeModifiers() {
    	...
    	if(upgradeSlotHandler.getStackInSlot(0).getItem().equals(ModItems.CAPACITY_UPGRADE.get())) {
    		...
    		inputFluidTank1.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), ItemStack.EMPTY, ItemStack.EMPTY, ItemStack.EMPTY);
    		inputFluidTank2.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), ItemStack.EMPTY, ItemStack.EMPTY, ItemStack.EMPTY);
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t3_slurry_processor");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT3SlurryProcessor(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, outputSlotHandler));
    ...
    public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
        ...
        if (!this.remove && side != null) {
			...
			if(cap == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T3SlurryProcessor.FACING) == Direction.NORTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T3SlurryProcessor.FACING) == Direction.SOUTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T3SlurryProcessor.FACING) == Direction.EAST) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T3SlurryProcessor.FACING) == Direction.WEST) {

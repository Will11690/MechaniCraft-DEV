...
public class T1SlurryProcessor extends Block {
	
    public static final DirectionProperty FACING = HorizontalBlock.FACING;
    public static final BooleanProperty LIT = BlockStateProperties.LIT;

    public T1SlurryProcessor(Properties properties) {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        ...
        return TileEntityHandler.TILE_ENTITY_T1_SLURRY_PROCESSOR.get().create();
    	...
    	if(heldItem.getItem() instanceof BucketItem || heldItem.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY).isPresent()) {
    		...
    		if(fluid.isPresent()) {
    			...
    			if(facing == Direction.NORTH) {
    				...
    				if(TileEntityT1SlurryProcessor.isRecipeInputFluid1(world, fluidStack.getFluid())) {
        			
    					return Direction.EAST;
        			
    				} else if(TileEntityT1SlurryProcessor.isRecipeInputFluid2(world, fluidStack.getFluid())) {
    ...
    public ActionResultType use(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult rayTrace) {
        ...
        if (!(te instanceof TileEntityT1SlurryProcessor)) {
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
    	 ...
    	 if(!state.is(newState.getBlock())) {
             ...
             if (tileEntity != null) {
            	 ((TileEntityT1SlurryProcessor) tileEntity).blockBeingBroken(true);
                 ...
                 cap.ifPresent(handler -> {
                	...
                	((TileEntityT1SlurryProcessor) tileEntity).blockBeingBroken(false);

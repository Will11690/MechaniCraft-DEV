...
public class T1ItemChute extends Block {
	...
	//BLOCK CONNECTION BOOLS
		...
		private static final VoxelShape DOWN_SHAPE_BLOCK = Shapes.join(Block.box(6, 3, 6, 10, 6, 10), Block.box(5, 0, 5, 11, 3, 11), BooleanOp.OR);
		private static final VoxelShape UP_SHAPE_BLOCK = Shapes.join(Block.box(6, 10, 6, 10, 13, 10), Block.box(5, 13, 5, 11, 16, 11), BooleanOp.OR);
		private static final VoxelShape NORTH_SHAPE_BLOCK = Shapes.join(Block.box(6, 6, 3, 10, 10, 6), Block.box(5, 5, 0, 11, 11, 3), BooleanOp.OR);
		private static final VoxelShape SOUTH_SHAPE_BLOCK = Shapes.join(Block.box(6, 6, 10, 10, 10, 13), Block.box(5, 5, 13, 11, 11, 16), BooleanOp.OR);
		private static final VoxelShape EAST_SHAPE_BLOCK = Shapes.join(Block.box(10, 6, 6, 13, 10, 10), Block.box(13, 5, 5, 16, 11, 11), BooleanOp.OR);
		private static final VoxelShape WEST_SHAPE_BLOCK = Shapes.join(Block.box(3, 6, 6, 6, 10, 10), Block.box(0, 5, 5, 3, 11, 11), BooleanOp.OR);
	
	
	public T1ItemChute(Properties properties) {
...
this.registerDefaultState(this.stateDefinition.any()
	...
	public static boolean canChuteConnectChute(IBlockReader world, BlockPos pos, Direction facing) {
		...
		if(neighborBlock == ModBlocks.T1_ITEM_CHUTE.get()) {
	...
	public static boolean canChuteConnectBlock(IBlockReader world, BlockPos pos, Direction facing) {
		...
		if(neighborBlock.hasTileEntity(neighborState) && neighborEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, facing).isPresent() && neighborBlock != ModBlocks.T1_ITEM_CHUTE.get()) {
...
return TileEntityHandler.TILE_ENTITY_T1_ITEM_CHUTE.get().create();

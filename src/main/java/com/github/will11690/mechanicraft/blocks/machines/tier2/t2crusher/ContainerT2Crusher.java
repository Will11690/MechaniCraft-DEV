...
public class ContainerT2Crusher  extends Container {
	
	private final IItemHandler playerInventory;
	private PlayerEntity player;
    private IItemHandler handler;
    private IIntArray fields;
    private TileEntityT2Crusher tile;
    private int energyStored;
    private int energyCapacity;

    public ContainerT2Crusher(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
        this((TileEntityT2Crusher) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(5));
        
    }

    public ContainerT2Crusher(@Nullable TileEntityT2Crusher tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T2_CRUSHER.get(), id);
        ...
        this.addSlot(new SlotItemHandler(this.handler, 1, 50, 32) {
        	...
        	public boolean mayPlace(ItemStack input) {
        		...
        		Optional<CrusherRecipes> recipe = TileEntityT2Crusher.getRecipeForInput(tile.getLevel(), input);
        		
        		if(recipe.isPresent() && !(ItemStack.matches(TileEntityT2Crusher.getResultForItem(tile.getLevel(), input), input))) {

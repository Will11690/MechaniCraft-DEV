...
public class TileEntityT1Press extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	public TileEntityT1Press() {

		super(TileEntityHandler.TILE_ENTITY_T1_PRESS.get());
	...
	private boolean canPress() {
		...
		if(allSlots.isPresent()) {
			...
			if(recipe != null && (output.getItem().equals(outputHandler.getItem()) || outputHandler.equals(ItemStack.EMPTY)) && (output.getCount() + outputHandlerCount <= outputHandler.getMaxStackSize())) {
				...
				if(energyStorage.getEnergyStored() >= pressingEnergy) {
	...
	private void startPressing() {
		...
		if(canPress()) {
			...
			if(progress < WORK_TIME) {
				...
				++progress;
				energyStorage.consumeEnergy(pressingEnergy);
	...
	public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
		...
		assert level != null;
		return new ContainerT1Press(this, this.fields, id, playerInventory, new CombinedInvWrapper(chargeSlotHandler, inputSlotHandler, outputSlotHandler));
	...
	public void load(BlockState state, CompoundNBT tags) {
		...
		this.pressingEnergy = tags.getInt("PressingEnergy");
	...
	public CompoundNBT save(CompoundNBT tags) {
		...
		tags.putInt("PressingEnergy", pressingEnergy);
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t1_press");

...
public class TileEntityEliteFluidTank extends TileEntity implements ITickableTileEntity {
    ...
    public static final int capacity = ModConfigs.eliteTankCapacityInt;

    public TileEntityEliteFluidTank() {
		
		super(TileEntityHandler.TILE_ENTITY_ELITE_FLUID_TANK.get());
	...
	public void tick() {
		...
		if(this.getBlockState().getValue(EliteFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(true)) {
				...
				if(neighborEntity.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY).isPresent()) {
					...
					if(this.fluidHandler.isPresent()) {
						...
						if(fluidTank.getFluidInTank(0).getAmount() > 0) {
							...
							FluidUtil.tryFluidTransfer(neighborFluidHandler, outputFluidHandler, Math.min(fluidTank.getFluidInTank(0).getAmount(), FluidAttributes.BUCKET_VOLUME * 4), true);
						}
					}
				}
			}
	...
	void setOutputBottomTrue() {
		...
		if(this.getBlockState().getValue(EliteFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(false)) {
			
			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(EliteFluidTank.OUTPUT_BOTTOM, Boolean.valueOf(true)));
		}
	}
	...
	void setOutputBottomFalse() {
		...
		if(this.getBlockState().getValue(EliteFluidTank.OUTPUT_BOTTOM) == Boolean.valueOf(true)) {
			...
			this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(EliteFluidTank.OUTPUT_BOTTOM, Boolean.valueOf(false)));
		}
	}
    ...
    public void load(BlockState state, CompoundNBT tags) {
        ...
        if(tags.contains("eliteFluidTank", Constants.NBT.TAG_COMPOUND)) {
			
            CompoundNBT fluidTankTag = tags.getCompound("eliteFluidTank");
    ...
    public CompoundNBT save(CompoundNBT tags) {
        ...
        tags.put("eliteFluidTank", fluidTankTag);
    ...
    public void onDataPacket(NetworkManager net, SUpdateTileEntityPacket packet) {
    	...
    	fluidTank.readFromNBT(packet.getTag().getCompound("eliteFluidTank"));

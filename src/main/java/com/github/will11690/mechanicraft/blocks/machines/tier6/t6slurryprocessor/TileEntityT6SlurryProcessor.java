package com.github.will11690.mechanicraft.blocks.machines.tier6.t6slurryprocessor;
...
public class TileEntityT6SlurryProcessor extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
	...
	private int processingEnergy = ModConfigs.t6SlurryProcessorEnergyPerTickInt/*PER TICK*/;
	private int WORK_TIME = ModConfigs.t6SlurryProcessorWorkTimeInt;
		
	private static int capacity = ModConfigs.t6SlurryProcessorEnergyCapacityInt;
	private static int receive = ModConfigs.t6SlurryProcessorReceiveInt;
	private static int fluid_capacity = ModConfigs.t6SlurryProcessorTankCapacityInt;
    ...
    public TileEntityT6SlurryProcessor() {
    	
        super(TileEntityHandler.TILE_ENTITY_T6_SLURRY_PROCESSOR.get());
    ...
    public void tick() {
        ...
        if(canCraft() && this.level.getBlockState(this.worldPosition).getValue(T6SlurryProcessor.LIT) == false) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6SlurryProcessor.LIT, Boolean.valueOf(true)));
        	
        }
        
        if(!canCraft() && this.level.getBlockState(this.worldPosition).getValue(T6SlurryProcessor.LIT) == true) {
        	
        	this.level.setBlockAndUpdate(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(T6SlurryProcessor.LIT, Boolean.valueOf(false)));
    ...
    private void setUpgradeModifiers() {
    	...
    	if(upgrade.isPresent()) {
    		...
    		if(progress == 0) {
        		...
        		upgradeHandler.fourUpgradeModifier(WORK_TIME, processingEnergy, upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
    	...
    	upgradeSlotHandler.getStackInSlot(2).getItem().equals(ModItems.CAPACITY_UPGRADE.get()) || upgradeSlotHandler.getStackInSlot(3).getItem().equals(ModItems.CAPACITY_UPGRADE.get())) {
    		...
    		inputFluidTank1.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
    		inputFluidTank2.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
    		outputFluidTank.upgradeCapacity(upgradeSlotHandler.getStackInSlot(0), upgradeSlotHandler.getStackInSlot(1), upgradeSlotHandler.getStackInSlot(2), upgradeSlotHandler.getStackInSlot(3));
	...
	public ITextComponent getDisplayName() {
		...
		return new TranslationTextComponent("container.mechanicraft.t6_slurry_processor");
    ...
    public Container createMenu(int id, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        ...
        assert level != null;
		return new ContainerT6SlurryProcessor(this, this.fields, id, playerInventory, new CombinedInvWrapper(upgradeSlotHandler, chargeSlotHandler, outputSlotHandler));
    ...
    public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
        ...
        if (!this.remove && side != null) {
			...
			if(cap == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6SlurryProcessor.FACING) == Direction.NORTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6SlurryProcessor.FACING) == Direction.SOUTH) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6SlurryProcessor.FACING) == Direction.EAST) {
				...
				if(this.level.getBlockState(this.worldPosition).getValue(T6SlurryProcessor.FACING) == Direction.WEST) {

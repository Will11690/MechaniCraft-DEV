...
public class ContainerT1MetallicInfuser extends Container {
	
	private final IItemHandler playerInventory;
	private PlayerEntity player;
    private IItemHandler handler;
    private IIntArray fields;
    private TileEntityT1MetallicInfuser tile;
    private int energyStored;
    private int energyCapacity;

    public ContainerT1MetallicInfuser(int id, PlayerInventory playerInventory, PacketBuffer exData) {
    	
    	this((TileEntityT1MetallicInfuser) playerInventory.player.level.getBlockEntity(exData.readBlockPos()), new IntArray(2), id, playerInventory, new ItemStackHandler(4));
    	
    }
    
    public ContainerT1MetallicInfuser(@Nullable TileEntityT1MetallicInfuser tile, IIntArray fields, int id, PlayerInventory playerInventory, IItemHandler iItemHandler) {
    	
        super(ContainerTypeHandler.CONTAINER_T1_METALLIC_INFUSER.get(), id);
        ...
        this.addSlot(new SlotItemHandler(this.handler, 1, 38, 32) {
        	
            @Override
            public boolean mayPlace(ItemStack stack) {
            	
            	if(TileEntityT1MetallicInfuser.isRecipeInput1(tile.getLevel(), stack)) {
        ...
        this.addSlot(new SlotItemHandler(this.handler, 2, 66, 32) {
        	
            @Override
            public boolean mayPlace(ItemStack stack) {
            	
            	if(TileEntityT1MetallicInfuser.isRecipeInput2(tile.getLevel(), stack)) {
        ...
        this.addSlot(new SlotItemHandler(this.handler, 3, 132, 32) {

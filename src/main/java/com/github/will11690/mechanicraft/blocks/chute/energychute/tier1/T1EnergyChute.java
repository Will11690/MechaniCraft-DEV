...
public class T1EnergyChute extends Block {
	...
	public static boolean canChuteConnectBlock(IBlockReader reader, BlockPos pos, Direction direction) {
		...
		if(neighborBlock.hasTileEntity(neighborState) && neighborEntity.getCapability(CapabilityEnergy.ENERGY, direction.getOpposite()).isPresent() && neighborBlock != ModBlocks.T1_ENERGY_CHUTE.get()) {
